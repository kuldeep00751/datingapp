            ...
            <div class="item-widget item-white-bg item-wg-title-icon-bg fadeIn" data-effect="fadeIn">
                ...
                @if (session('error-comment'))
                    <div class="alert badge-danger text-white" role="alert">
                        {{ session('error-comment') }}
                    </div>
                @endif
                <div class="item-widget-main-content">
                    <div class="item-widget-head">
                        <h2 class="item-widget-title mb-3">
                            <i class="fas fa-info float-left"></i>@lang('messages.comment-title')
                        </h2>
                    </div>
                    <div class="item-widget-content">
                        <div class="item-infos-content">
                            <div class="mb-3">
                                 <h5>@lang('messages.comment-top-message')</h4>
                            </div>
                            <form method="post" action="{{ route('meeting.setComments') }}" enctype="multipart/form-data">
                                @csrf
                                
                                <!-- Picture Option -->
                                <div class="mb-3">
                                    <h5>
                                        <input type="checkbox" id="pictureCheckbox" name="visual_picture" onclick="toggleReasonSelect()" {{ optional($comments)->visualPicture == 1 ? 'checked' : '' }}> @lang('messages.comment-visual-picture')
                                    </h5>
                                </div>
                                <div class="mb-3" id="reasonSelectContainer" style="display: none;">
                                    @php
                                        $pictures = auth()->user()->pictures()->latest()->get();
                                    @endphp
                                    @foreach($pictures as $key=>$value)
                                        <h5 class="m-2">
                                            <input type="checkbox" name="reason_profile{{$key}}" {{ optional($comments)->{"reason_profile$key"} == 1 ? 'checked' : '' }} 
                                            onclick="toggleProfileContainer({{ $key }})">
                                ...
                                <div class="mb-3">
                                    <h5>
                                        <input type="checkbox" id="descriptionCheckbox" name="visual_description" onclick="toggleDescriptionSelect()" {{ optional($comments)->visualDescription == 1 ? 'checked' : '' }}> @lang('messages.comment-description')
                                    </h5>
                                </div>
                                <div class="mb-3" id="descriptionSelectContainer" style="display: none;">
                                    <textarea class="form-control" name="reason_description" id="reasonDescription1" rows="2" placeholder="">{{ optional($comments)->reason_description }}</textarea>
                                </div>

                                <div class="mb-3">
                                     <h5>@lang('messages.comment-additional-comment'):</h4>
                                    <textarea class="form-control" id="comments1" name="comments" rows="3" placeholder="{{ __('messages.comment-additional-comment-placeholder')}}..." required>{{ optional($comments)->comments}}</textarea>
                                </div>
                                @if($comments->visualPicture ==0 && $comments->visualDescription == 0)
                                <div class="text-left">
                                    <button type="submit" class="btn btn-danger btn-lg">@lang('messages.btn-submit')</button>
                                </div>
                                @endif
                            </form>
                        </div>
...
<script>
     window.onload = function() {
        ...
        var pictureCheckbox = document.getElementById('pictureCheckbox');
        var reasonSelectContainer = document.getElementById('reasonSelectContainer');
        
        if (pictureCheckbox.checked) {
            reasonSelectContainer.style.display = 'block';  // Show the reason container if checked
        }

        var checkbox = document.getElementById('descriptionCheckbox');
        var container = document.getElementById('descriptionSelectContainer');
        
        // Show or hide the container based on whether the checkbox is checked
        if (checkbox.checked) {
            container.style.display = 'block'; // Show the container
        }

        @foreach($pictures as $key=>$value)
            var checkbox = document.getElementById('reason_profile{{$key}}');
            var container = document.getElementById('profileContainer{{$key}}');
            
            if (checkbox.checked) {
                container.style.display = 'block';  // Show the container if checkbox is checked
            }
        @endforeach
        toggleReasonSelect();
        toggleDescriptionSelect();
    }
    // Function to toggle the visibility of the reason container for the 'Visual Chemistry' section
    function toggleReasonSelect() {
        var checkbox = document.getElementById('pictureCheckbox');
    ...
    // Function to toggle the visibility of the description container
    function toggleDescriptionSelect() {
        var checkbox = document.getElementById('descriptionCheckbox');
        var container = document.getElementById('descriptionSelectContainer');
