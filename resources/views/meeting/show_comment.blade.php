...
<div id="info" class="row d-flex justify-content-center w-100 my-5">
   <div class="col-lg-5 col-md-5 col-sm-12" data-effect="fadeIn">
      <div class="form-section">
         <div class="item-widget-head" style="font-family: 'AvenirNext', sans-serif;">
            <h3 class="item-widget-title mb-3 text-white">
               @lang('messages.show_comment_1')
               <div class="feedback-title-comment mt-3">@lang('messages.show_comment_2') <br><br><span style="color: #73c0f7db;">@lang('messages.show_comment_3').</span></div>
            </h3>
         </div>
         @if($commentsLast->isNotEmpty())
         <div class="item-widget-content">
            @foreach($commentsLast as $comments)
               @if($comments->comments !=null)
               @php 
               $commentUserName = getUserDetails($comments->liked_user_id)->like_to_be_called ?? '';
               @endphp
               <div class="item-infos-content pt-3" style="font-family: 'FutureBTBook', sans-serif;border-top: 2px solid #fff;">
                  <div class="mb-3 text-white">
                     <h6>
                     @lang('messages.show_comment_4') {{ $commentUserName }}.
                     </h6>
                  </div>
                  @if($comments->visualPicture == 1)
                  <div class="mb-3 text-white">
                     <h5>@lang('messages.show_comment_8'):</h5>
                     <div class="mb-4 row mx-2">
                        @if(isset(auth()->user()->id))
                        @php $images = auth()->user()->pictures()->get(); @endphp
                        @foreach($images as $key => $value)
                        @php $field = 'reason_profile' . $key; @endphp
                        @if(isset($comments->$field) && $comments->$field == 1)
                        <img class="mx-3" src="{{ asset('storage/' . $value->picture_location) }}" width="100" style="border: 2px solid #fff;" />
                        @endif
                        @endforeach
                        @endif
                     </div>
                  ...
                  @endif
                  @if($comments->visualDescription == 1)
                  <div class="mb-4 text-white">
                     <h5>@lang('messages.show_comment_5'):</h5>
                     <p style="word-break: break-word;"> {{ optional($comments)->reason_description }}</p>
                  </div>
                  @endif
                  <div class="mb-4 text-white">
                     <h5>
                     @lang('messages.show_comment_6'):</h4>
                     <p style="word-break: break-word;">{{ optional($comments)->comments}}</p>
                  </div>
               </div>
               <hr>
               @endif
         ...
         @else
         <div class="item-widget-content pt-3" style="font-family: 'FutureBTBook', sans-serif;border-top: 2px solid #fff;">
            <div class="item-infos-content">
               <div class="mb-3 text-white">
                  <h6>@lang('messages.show_comment_7')</h6>
               </div>
            </div>
         </div>
         @endif
