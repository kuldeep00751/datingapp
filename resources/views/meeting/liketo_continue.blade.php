    ...
    <div class="form-section" id="info" style="color:#fff !important;">
        <div class="item-widget-content dataflow">
            <div class="item-infos-content">
                ...
                <form method="post" action="{{ route('meeting.storelikeToContinue') }}"  enctype="multipart/form-data" id="meetingForm">
                    @csrf
                    <div class="mb-4 text-center" >
                        <h3 style="color:#fff !important;">@lang('messages.like_to_continue_1')</h3>
                    </div>
                    @if($message == true)
                    <div class="item-infos-item my-2 heading">
                        <div class="item-info-label">
                            
                            <span class="heading-span">@lang('messages.like_to_continue_2').</span><br></br>

                            <span class="heading-span">@lang('messages.like_to_continue_3').
                            </span> <br></br>

                            <span>
                            @lang('messages.like_to_continue_4').
                            </span>
                        </div>
                    </div>  
                @else
                    <div class="item-infos-item">
                        <div class="item-info-label" ><span style="color:#fff !important;" class="heading-span">@lang('messages.like_to_continue_5'):</span></div>
                    </div>
                    <div class="mb-4">
                        <label for="continue_meet" class="form-label">@lang('messages.like_to_continue_6')</label>
                        <select class="form-select" name="continue_meet" required>
                            <option value="yes">@lang('messages.like_to_continue_7')</option>
                            <option value="no">@lang('messages.like_to_continue_8')</option>
                        </select>
                    </div>
                    <div class="submit-section text-center">
                        <button type="submit" class="btn btn-danger btn-lg">
                            <i class="fas fa-paper-plane me-2"></i> @lang('messages.like_to_continue_9')
                        </button>
                    </div>
                @endif
                </form>
...
$(document).ready(function () {
    $('#meetingForm').on('submit', function (e) {
        ...
        var continue_meet = $('select[name="continue_meet"]').val();
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                if (response.success) {
                    Swal.fire({
                        icon: 'success',
                        title: '{{ __('messages.like_to_continue_10') }}',
                        text: '{{ __('messages.like_to_continue_11') }}',
                    }).then(() => {
                    ...
                    Swal.fire({
                        icon: 'error',
                        title: '{{ __('messages.like_to_continue_12') }}',
            ...
            error: function (xhr, status, error) {
                Swal.fire({
                    icon: 'error',
                    title: '{{ __('messages.like_to_continue_12') }}',
                    text: '{{ __('messages.like_to_continue_13') }}',
                    });
            }
        });
    });
